...
CombatEngine::CombatEngine(TArray<UGameCharacter*> playerParty, TArray<UGameCharacter*> enemyParty)
{
	...
	//  set combat instance of all characters to point to itself
...
bool CombatEngine::Tick(float DeltaSeconds)
{
	...
	{
		//selects character if not waiting, once decision is finished goes to next character or switches to action phase
	...
	{
		//selects character if not waiting, once action is finished goes to next character or switches to decision phase
		if (!this->waitingForCharacter)
		{
...
void CombatEngine::SelectNextCharacter()
{
	...
	{
		//changed GameCharacter -> UGameCharacter
